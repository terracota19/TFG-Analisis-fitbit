...
class App(tk.Tk):
    ...
    def create_register_link(self, frame):
        container = tk.Frame(frame, bg="#9a5bc1")
        container.pack(pady=10, padx=20, fill=tk.X)
        register_label = tk.Label(container, text="¿No tienes cuenta? Regístrate aquí", fg="white", cursor="hand2", bg="#9a5bc1", font=('arial', 12))
    ...
    def create_button(self, frame, text, command, state="normal", label_var=None):
        ...
        button = tk.Button(container, text=text, fg="white", bg="#9a5bc1", state=state, command=lambda: self.button_click(command, label_var), font=("arial", 12))
        button.pack(side=tk.LEFT)  
        if label_var is not None:
            label = tk.Label(container, textvariable=label_var, fg="green", bg="#9a5bc1")
