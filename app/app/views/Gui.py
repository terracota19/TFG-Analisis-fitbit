...
class App(tk.Tk):
    ...
    def show_error_message(self, frame, text):
        error_label = tk.Label(frame, text=text, font=("arial", 17), fg="red")
    ...
    def create_status_frame(self, frame):
        ...
        self.update_label = self.create_label(self.status_frame, f"Última actualización: {last_update}", 14, fg="white", bg="#457EAC")
    ...
    def run_update_status(self):
        ...
        if not self.ult_act:
            dates = self.get_last_30_days() #si es la primera que actualiza pedimos los 30 dias anteriores si existe sino nada.
        else :
            dates = self.get_dates_since_last_activity(self.ult_act) #si tiene fecha de actualzacion, coger desde ese dia hasta el dia de hoy, sin importar
            #que haya varios meses desde la ultima que actualizo
    ...
    def get_dates_since_last_activity(ult_act):
        
        last_activity_date = datetime.fromisoformat(ult_act)        
        today = datetime.today()
