...
class App(tk.Tk):
    ...
    def selectUserData(self):
        ...
        "smiley icon"
        self.smiley_icon = Image.open('app/assets/images/smiley.png')
        self.smiley_icon= self.smiley_icon.resize((30, 30), Image.LANCZOS)  
        self.smiley_icon = ImageTk.PhotoImage(self.smiley_icon)

        self.smiley_button = tk.Button(self.mostrar_frame, image=self.smiley_icon, bg="#626CC2", relief=tk.FLAT, command=self.close_settings_tab)
        self.smiley_button.image = self.smiley_button 
        self.smiley_button.pack(side=tk.RIGHT)
    ...
    def graphPredictions(self, predictions):
        ...
        self.sad_icon = Image.open('app/assets/images/sad.png')
        self.sad_icon= self.sad_icon.resize((30, 30), Image.LANCZOS)  
        self.sad_icon = ImageTk.PhotoImage(self.sad_icon)

        if self.insidePreferedHeartRateZone(predictions):
            self.range_label = tk.Label(self.predict_range_graph_frame, text="¡Sigue asi! ", fg="white", bg="#626CC2", font=("Segoe UI", 14), image = self.smiley_icon, compound = "right")
            self.range_label.pack(side=tk.RIGHT, pady=5)
            self.predict_range_graph_frame.pack()
        else :
            self.range_label = tk.Label(self.predict_range_graph_frame, text="¡Poner algo! ", fg="white", bg="#626CC2", font=("Segoe UI", 14), image = self.sad_icon, compound = "right")
            self.range_label.pack(side=tk.RIGHT, pady=5)
            self.predict_range_graph_frame.pack()
    ...
    def insidePreferedHeartRateZone(self, predictions):
        ...
        print(preferencia, FCM_value, FCR_value)
