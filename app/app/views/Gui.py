...
class App(tk.Tk):
    def __init__(self):
        ...
        self.from_date = None
        self.to_date = None
        self.to_hour_var = None
        self.from_hour_var = None
    ...
    def createLastUpdateLabel(self, frame, last_update):
        if last_update != "Nunca":
            ...
            self.update_label = self.create_label(frame, f"Última Sincronización con la pulsera: Nunca", 14, fg="grey", bg="white")
    ...
    def create_register_frame(self):
        ...
        self.create_entry(self.register_frame, "Usuario: (*)", False, 'register_user_entry', "<KeyPress-Return>", "on_register_submit")
        self.create_entry(self.register_frame, "Correo: (*)", False, 'register_email_entry', "<KeyPress-Return>", "on_register_submit")
        self.create_entry(self.register_frame, "Contraseña: (*)", True, 'register_password_entry', "<KeyPress-Return>", "on_register_submit")
    ...
    def showFilterOptions(self):
        ...
        self.filter_popup.geometry("450x150")
        
        tk.Label(self.filter_popup, text="Desde:", font=("Segoe UI", 14), fg="black", bg="white").grid(row=0, column=0, padx=5, pady=5, sticky=tk.E)
        self.from_date = DateEntry(self.filter_popup)
        self.from_date.grid(row=0, column=1, padx=5, pady=5)

        tk.Label(self.filter_popup, text="Hora:", font=("Segoe UI", 14), fg="black", bg="white").grid(row=0, column=2, padx=5, pady=5, sticky=tk.E)
        self.from_hour_spinbox = tk.Spinbox(self.filter_popup, from_=0, to=23, width=5, format="%02.0f",wrap=True)
        self.from_hour_spinbox.grid(row=0, column=3, padx=5, pady=5)

        tk.Label(self.filter_popup, text="Minuto:", font=("Segoe UI", 14), fg="black", bg="white").grid(row=0, column=4, padx=5, pady=5, sticky=tk.E)
        self.from_minute_spinbox = tk.Spinbox(self.filter_popup, from_=0, to=59, width=5, format="%02.0f", wrap=True)
        self.from_minute_spinbox.grid(row=0, column=5, padx=5, pady=5)

        tk.Label(self.filter_popup, text="Hasta:", font=("Segoe UI", 14), fg="black", bg="white").grid(row=1, column=0, padx=5, pady=5, sticky=tk.E)
        self.to_date = DateEntry(self.filter_popup)
        self.to_date.grid(row=1, column=1, padx=5, pady=5)


        tk.Label(self.filter_popup, text="Hora:", font=("Segoe UI", 14), fg="black", bg="white").grid(row=1, column=2, padx=5, pady=5, sticky=tk.E)
        var_hour= tk.StringVar()
        var_hour.set(23)
        self.to_hour_spinbox = tk.Spinbox(self.filter_popup, from_=0, to=23, width=5, format="%02.0f",wrap=True, textvariable = var_hour)
        
        self.to_hour_spinbox.grid(row=1, column=3, padx=5, pady=5)

        tk.Label(self.filter_popup, text="Minuto:", font=("Segoe UI", 14), fg="black", bg="white").grid(row=1, column=4, padx=5, pady=5, sticky=tk.E)
        var_minute= tk.StringVar()
        var_minute.set(59)
        self.to_minute_spinbox = tk.Spinbox(self.filter_popup, from_=0, to=59, width=5, format="%02.0f",wrap=True, textvariable = var_minute)
        self.to_minute_spinbox.grid(row=1, column=5, padx=5, pady=5)

        self.select_frame = tk.Frame(self.filter_popup, bg="white")
        self.select_date_range = tk.Label(self.select_frame, text="Seleccionar", fg="black", bg="white", cursor="hand2", font=('Segoe UI', 12))
        self.select_date_range.pack(pady=5)
        self.select_date_range.bind("<Button-1>", lambda e: self.selectDateRange())

        self.select_frame.grid(row=2, column=0, columnspan=6, pady=10, sticky=tk.NSEW)
