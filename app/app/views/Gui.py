...
class App(tk.Tk):
    def __init__(self):
        ...
        """Calendar"""
        self.from_calendar_popup = None
        self.to_calendar_popup = None
        self.from_date = None
        self.to_date = None
        self.from_cal = None
        self.to_cal = None
        ...
        self.from_calendar_icon = None
        self.to_calendar_icon = None
    ...
    def create_prediction_frame(self):
        ...
        self.create_label(self.prediction_comboBox_frame, "Selecciona el tipo de datos y los minutos a predecir", 14)

        """SubFrame"""
        self.combobox_frame = tk.Frame(self.prediction_comboBox_frame, bg="#626CC2")
        self.combobox_frame.pack()

        self.dataPredComboBox = ttk.Combobox(self.combobox_frame, values=["HeartRate","Calories","Distance","Steps"])
        ...
        self.spinBox = tk.Spinbox(self.combobox_frame, from_=1, to=100, increment=1)
        #self.comboBox.set("Selecciona minutos")
      
        self.comboBox.grid(row=0, column=1, padx=20, pady=5) 
    ...
    def selectUserData(self):
        ...
        """Frame"""
        self.calendars_frame = tk.Frame(self.dataUser_frame, bg="#626CC2")
        
        """SubFrames"""
        self.from_calendars_frame = tk.Frame(self.calendars_frame, bg="#626CC2")
        self.to_calendars_frame = tk.Frame(self.calendars_frame, bg="#626CC2")

        
        #createFromCalendar
        self.from_calendar_icon = Image.open('app/assets/images/calendar.png')
        self.from_calendar_icon = self.from_calendar_icon.resize((30, 30), Image.LANCZOS)  
        self.from_calendar_icon = ImageTk.PhotoImage(self.from_calendar_icon)

        self.from_calendar_button = tk.Button(self.from_calendars_frame, image=self.from_calendar_icon, bg="#626CC2", 
                                              relief=tk.FLAT,highlightthickness=3, highlightbackground="white",command=self.createFromCalendar)
        self.from_calendar_button.image = self.from_calendar_button 


        #createToCalendar
        self.to_calendar_icon = Image.open('app/assets/images/calendar.png')
        self.to_calendar_icon = self.to_calendar_icon.resize((30, 30), Image.LANCZOS)  
        self.to_calendar_icon = ImageTk.PhotoImage(self.to_calendar_icon)

        self.to_calendar_button = tk.Button(self.to_calendars_frame, image=self.to_calendar_icon, bg="#626CC2", 
                                            highlightthickness=3, highlightbackground="white", relief=tk.FLAT, command=self.createToCalendar)
        self.to_calendar_button.image = self.to_calendar_button 
       

        self.from_label = self.create_label(self.from_calendars_frame,"Desde:", 14)
        self.from_label.pack(side=tk.LEFT, pady = 5)
        self.from_calendar_button.pack(side=tk.LEFT, pady = 5)

        
        self.to_label = self.create_label(self.to_calendars_frame,"Hasta:", 14)
        self.to_label.pack(side=tk.LEFT, pady = 5)
        self.to_calendar_button.pack(side=tk.LEFT, pady = 5)

        self.from_calendars_frame.grid(row=0, column=0, padx=20, pady=5)
    ...
    def createFromCalendar(self):
        self.from_calendar_popup = tk.Toplevel(self, bg="white")
        self.from_calendar_popup.title("Elige una fecha")
        self.from_calendar_popup.geometry("300x250")      

        self.from_cal = Calendar(self.from_calendar_popup, selectmode='day', year=datetime.today().year, month=datetime.today().month, day=datetime.today().day,
                        background="white", foreground='black', selectbackground='#626CC2')
        self.from_cal.pack(pady=5) 

        
        from_select_link = tk.Label(self.from_calendar_popup, text="Seleccionar", fg="black", bg="white", cursor="hand2", font=('Segoe UI', 12),
                                    highlightthickness=1, highlightbackground="black")
        from_select_link.pack(pady=5)
        from_select_link.bind("<Button-1>", lambda e: self.getFromDate())

        
    def getFromDate(self):
        self.from_date = self.from_cal.get_date()
        self.from_calendar_popup.destroy()

    def createToCalendar(self):
        self.to_calendar_popup = tk.Toplevel(self, bg="white")
        self.to_calendar_popup.title("Elige una fecha")
        self.to_calendar_popup.geometry("300x250")      

        self.to_cal = Calendar(self.to_calendar_popup, selectmode='day', year=datetime.today().year, month=datetime.today().month, day=datetime.today().day,
                        background="white", foreground='black', selectbackground='#626CC2')
        self.to_cal.pack(pady=5) 
        
        to_select_link = tk.Label(self.to_calendar_popup, text="Seleccionar", fg="black", bg="white", cursor="hand2", font=('Segoe UI', 12),
                                    highlightthickness=1, highlightbackground="black")
        to_select_link.pack(pady=5)
        to_select_link.bind("<Button-1>", lambda e: self.getToDate())

        
    def getToDate(self):
        self.to_date = self.to_cal.get_date()
        self.to_calendar_popup.destroy()
    """
