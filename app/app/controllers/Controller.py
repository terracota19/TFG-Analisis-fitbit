...
class Controller:
    ...
    def lastMongoUpdate(self):
        ...
        if user_id:
            ...
            return None
    """
        Logic that checks whether user with email exists in MongoDB or user with Fitbit user_id is already registered in MongoDB.

        Parameters:
        -email (str (Unique)) : User registered email.
        -user_id (str (Unique)) : Fitbit User id.

        Returns (UserRegistrationError) if user with email exists in MongoDB.
                (UserFitbitAccountAlreadyInUseError) if Fitbit user_id is already registered in MongoDB.
    """
