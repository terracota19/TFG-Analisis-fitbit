...
class Controller:
    ...
    def register(self, user, email, password, edad, user_id, access_token, refresh_token, expires_in):
        if not (user and email and password and edad):
            ...
            supposedUser = self.checkIfUserExists(email)

            if supposedUser:
                raise UserRegistrationError(f"El usuario con correo : {email} ya existe!")
    ...
    def checkIfUserExists(self, email):
        return self.mongo.find_one_data("usuarios", query={"correo" : email})
