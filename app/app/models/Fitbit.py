...
class FitbitAPI:
    ...
    def combine_monthly_data(self, base_filename, output_filename):
        ...
        for month in range(1, 13):
            filename = f"app/app/DataAPI/{self.user_id}/{base_filename}_{datetime.today().month}.csv"
            try:
    ...
    def dataPreprocess(self):
        ...
        self.combine_monthly_data("calories_data", f"app/app/DataAPI/{self.user_id}/calories_merged.csv")
        self.combine_monthly_data("distance_data", f"app/app/DataAPI/{self.user_id}/distance_merged.csv")
    ...
    def access_token_is_expired(self, access_token, expires_in):
        if access_token and expires_in is not None:
            ...
            if isinstance(expires_in, str):
                expires_at_utc = datetime.strptime(expires_in, "%Y-%m-%d %H:%M:%S.%f").replace(tzinfo=timezone.utc)
                ...
                raise ValueError("expires_in must be a datetime object or a string")
    ...
    def guardar_en_csv(self, data):
        csv_filename = f'app/app/DataAPI/{self.user_id}/profile.csv'
    ...
    def getHeartRateData(self, detail_level, start_time, end_time, dates):
        ...
        all_heart_data =  self.fetchData(base_url, detail_level, start_time, end_time, dates, 
                                f"app/app/DataAPI/{self.user_id}/heart_rate_data_{datetime.today().month}.csv", 
                                ['Id', 'Date', 'Time', 'HeartRate'])
        #Guardar en un csv
        self.store_HeartRate_csv(all_heart_data,
                                 csv_filename=f"app/app/DataAPI/{self.user_id}/heart_rate_data_{datetime.today().month}.csv" ,
    ...
    def store_CaloriesDistanceSteps_csv(self,source, source_data, csv_filename, csv_headers):
        ...
        csv_file_path = f"app/app/DataAPI/{self.user_id}/{source}_data_{datetime.today().month}.csv"
    ...
    def getCaloriesDistanceStepsData(self, detail_level, start_time, end_time, dates):
        ...
        for source in data_sources:
            ...
            source_data = self.fetchData(base_url, detail_level, start_time, end_time, dates,
                                f"app/app/DataAPI/{self.user_id}/{source}_data_{datetime.today().month}.csv", 
                                ['Id', 'Date', 'Time', source.capitalize()])

            self.store_CaloriesDistanceSteps_csv(source,source_data,
                                 csv_filename=f"app/app/DataAPI/{self.user_id}/{source}_data_{datetime.today().month}.csv",
