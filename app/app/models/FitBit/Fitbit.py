from app.models.ML.MLP import MLP
...
class FitbitAPI:
    ...
    """
        Best (model, lags) for the number of steps minutes choosen.
        
        Parameters:
        -step: Number of minutes to predict into future.

        Returns:
        -Best (model, lags) for the number of steps minutes choosen.

    """
    def getBestModelAndLagsWith(self, step):
        if not step in self.bestModelAndLagsForSteps:
            raise PredictionError(f"Error en la predicción : opción {step} no válido")
        
        return self.bestModelAndLagsForSteps.get(step)

    """
        Logic that obtains data_title data with data_frec frecuency
    """
    def userData(self, data_title, data_frec):
        
        return self.dataHandler.userData(data_title, self.user_id, data_frec, self.access_token)
    ...
    """
        ...
        Returns:
        - list : Predictions by ML for every minuted selected by user.
    
    """
    def predictions(self, steps):
        model_name, _ = self.getBestModelAndLagsWith(steps)
        return self.models.get(model_name).predictions()
    ...
    """
    def datosReales(self, steps):
      
        model_name, _ = self.getBestModelAndLagsWith(steps)
        print(f"llamada en Fitbit datosReales {model_name}")
        return self.models.get(model_name).datosReales()
    ...
    """
        ...
        -new_expires_in (Datetime) : Number of seconds to expire user access_token
    ...
    """
        ...
        Returns:
        -(boolean) : if access token is expired or not. False Otherwise.
    """
    def access_token_is_expired(self, access_token, expires_in):   
        return self.auth.access_token_is_expired(access_token, expires_in)
